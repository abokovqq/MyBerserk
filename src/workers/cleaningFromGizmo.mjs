...
// ===== Онлайн посетители Gizmo по открытым сессиям =====
...
for (const sess of sessRows) {
  ...
  const onlineBadge = onlineVisitorsCount === null ? '👥 ?' : `👥 ${onlineVisitorsCount}`;

  const msg =
    `<b>${displayPlace}</b> ${userName} <b>${timeLabel}</b> ${onlineBadge}\n` +
    `❓ Статус уборки`;
